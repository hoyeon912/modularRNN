"""CNN-RNN-DQN on CartPole-v1: identical to scripts/test_dqn_cartpole.py's CNN-DQN (same
conv front-end, Double DQN target, n-step returns, LR decay, decoupled greedy evaluation)
except the head's last hidden layer is a 128-unit RNNCell whose hidden state persists across
timesteps within an episode, instead of a stateless Linear layer -- a "stored-state" DRQN
(Hausknecht & Stone, 2015).

Kept in its own file (reusing scripts/test_dqn_cartpole.py's frame rendering/stacking
helpers and get_device) rather than folded into test_dqn_cartpole.py, so the two Q-network
architectures -- and their results -- stay clearly distinguishable: this file's model has a
persistent hidden state to thread through training and evaluation, which the plain CNN-DQN
does not, and every function here (`optimize_model_recurrent`, `evaluate_greedy_recurrent`,
`train_dqn_recurrent`, ...) has to account for that.
"""
...
class CNNRNNDQN(nn.Module):
    """Same conv front-end as test_dqn_cartpole.DQN (matching CNNEncoder's kernel schedule),
    but the last hidden layer is an RNNCell (`rnn_hidden_size` units, 128 by default) whose
    hidden state persists across timesteps within an episode, instead of DQN's stateless
    Linear layer. `hidden_sizes` sets any Linear+ReLU layers *before* the RNN cell -- ()
    feeds conv features straight into the RNN cell, matching "best CNN-DQN"'s single-128-
    unit-hidden-layer head but with that layer replaced by an RNN instead of a Linear."""
...
def train_dqn_recurrent(
    ...
    every step (stored-state DRQN) instead of treating each frame stack as an independent
    Markov state. Each pushed transition carries the hidden state entering and leaving its
    n-step window, so replay can recompute Q-values without replaying full episodes.

    `n_step` and its end-of-episode flush behave exactly as in test_dqn_cartpole.train_dqn
    (see that docstring); the only difference here is each buffered entry also carries the
...
def test_cnn_rnn_dqn_step_shape_and_hidden_changes():
    torch.manual_seed(0)
    net = CNNRNNDQN(in_channels=4, n_actions=2, image_size=32, rnn_hidden_size=16)
...
def test_select_action_recurrent_is_greedy_at_zero_epsilon():
    ...
    net = CNNRNNDQN(in_channels=1, n_actions=3, image_size=16, rnn_hidden_size=8)
...
def test_optimize_model_recurrent_uses_double_dqn_target():
    ...
    # test_dqn_cartpole.test_optimize_model_uses_double_dqn_target, adapted to the
    # step(x, h) interface.
    device = torch.device("cpu")
...
def test_soft_update_recurrent_interpolates_towards_policy_net():
    policy_net = CNNRNNDQN(1, 2, image_size=16, rnn_hidden_size=8)
    target_net = CNNRNNDQN(1, 2, image_size=16, rnn_hidden_size=8)
    with torch.no_grad():
...
def test_evaluate_greedy_recurrent_runs_and_leaves_net_in_train_mode():
    ...
    net.train()
    durations = evaluate_greedy_recurrent(net, device, image_size=32, frame_stack=4, num_episodes=3)
    assert len(durations) == 3
    assert all(d >= 1 for d in durations)
    assert net.training  # must restore train() mode, since optimize_model_recurrent requires it


def test_train_dqn_recurrent_smoke_runs_on_cartpole():
    # Small image size / few episodes per CLAUDE.md's "use small size of hidden units in
    # simple test" -- checks the full loop (frame rendering, hidden-state threading,
    # replay, optimize_model_recurrent, soft target update) runs end-to-end without error.
    torch.manual_seed(0)
    random.seed(0)
    device = get_device()
    env = gym.make("CartPole-v1")
    try:
        num_episodes = 5
        durations, history = train_dqn_recurrent(
            env,
            device,
